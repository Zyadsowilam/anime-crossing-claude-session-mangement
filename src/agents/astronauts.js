...
import { nameFor, themeFor } from '../game/themes.js'
...
/**
 * Status colour, carried by the spirit orb that floats at each character's shoulder.
 *
 * In the space version this colour was the astronaut's whole trim, which worked because
 * every astronaut was otherwise the same white suit. Here it cannot be: the outfit belongs
 * to the character's genre, and repainting it by status would erase the one thing that says
 * which chat you are looking at. So status moves *off* the body and onto the orb, which is
 * unlit, pushed past 1.0 for the bloom pass, and therefore the brightest thing on a
 * character at any distance — a blocked thread still reads as red from across the map.
 */
const STATUS_AURA = {
  working: 0x4fd07a,
  waiting: 0x4f9eff,
  blocked: 0xff4f4f,
  celebrating: 0xffc94f,
  idle: 0x9b9bb0,
  sleeping: 0x6a6a9a,
  spawning: 0xff8a5a,
  leaving: 0x7f9f92,
...
const TURN_RATE = 7.5
/**
 * How fast you move. Faster than the crew walks — you are the one being waited for, and a
 * player who has to trail a wandering character across a plot at its own pace is a player
 * watching a loading bar.
 */
const PLAYER_WALK = 3.4
const PLAYER_RUN = 7.2
/**
 * Riding speed. Nearly twice a sprint, which turns a thirty-second crossing into about
 * twelve — long enough to still be a journey, short enough not to be a commute.
 */
const PLAYER_RIDE = 13.5
/**
 * How far above the ground the rider sits once there is something under them.
 *
 * Measured against the mount rather than guessed: its saddle blanket sits at about 0.99 in
 * its own space, and a character stood on that has its feet on the blanket. Too low and the
 * rider is buried to the waist in cat.
 */
const RIDE_LIFT = 1.02
/** How close you have to be to a character before they will talk to you. */
const TALK_RANGE = 3.2
/** How much of the crowd's separation force the player actually feels. See `_playerMove`. */
const PLAYER_PUSHBACK = 0.25
...
export class Astronauts {
  constructor(scene, settings) {
    ...
    // Head, chest and hand bones each need their world matrix held across the whole of one
    // character's write, because hair, face and prop all read from them in turn.
    this._m5 = new THREE.Matrix4()
    this._m6 = new THREE.Matrix4()
    this._m7 = new THREE.Matrix4()
    this._propC = new THREE.Color()
    ...
    /** Who is talking to whom, and who has wandered over to watch. See `social.js`. */
    this.social = new Social()
    this.nav = null
    /**
     * Where the picture is being taken from, for the draw-range cull in `_writeMatrices`.
     * Set by the colony each frame; null means draw everybody, which is what the tools and
     * the tests want.
     */
    this.viewPoint = null
  }
  ...
  _buildMeshes(capacity) {
    ...
    /**
     * Meshes that are *not* indexed by the agent's own slot, because only some characters
     * wear them: every hairstyle, every prop, and the hammer. Each keeps its own instance
     * counter per frame — an unused slot in the middle of an instanced mesh still draws,
     * so the eleven characters wearing twintails must occupy slots 0..10 of that mesh and
     * nothing else. `variantOf` maps a mesh name back to the key an agent is matched on.
     */
    this.variantOf = new Map()

    // Cel shading. One ramp texture shared by everything on a character, which is what
    // makes hair, skin and cloth read as parts of the same drawing rather than as three
    // separately lit objects sitting near each other.
    this.gradient = toonGradient()
    const toon = (extra = {}) =>
      new THREE.MeshToonMaterial({ color: 0xffffff, gradientMap: this.gradient, ...extra })

    const R = P.helmetR

    // The head, in the slot the helmet used to occupy. Skin tone arrives per character
    // through `instanceColor`, so one mesh covers every complexion.
    parts.head = this._mesh(headGeometry(), toon(), capacity, false)

    // Hair. One mesh per style: seven draw calls at absolute worst, and only for the styles
    // actually on screen, since a mesh whose count lands on zero is skipped by the renderer.
    for (const style of HAIR_STYLES) {
      const name = 'hair_' + style
      parts[name] = this._mesh(buildHair(style), toon(), capacity, true)
      this.variantOf.set(name, style)
    }

    // The ahoge, on everyone. Its own mesh because it is animated per character — see the
    // spring in `_writeMatrices` — rather than baked into each hairstyle.
    parts.ahoge = this._mesh(ahogeGeometry(), toon(), capacity, false)

    // Props, one mesh per kind, matched the same way hair is.
    for (const kind of PROP_KINDS) {
      const geo = buildProp(kind)
      if (!geo) continue
      const name = 'prop_' + kind
      parts[name] = this._mesh(geo, toon(), capacity, true)
      this.variantOf.set(name, kind)
    }

    // The spirit orb: status, as the one unlit thing on a character. Pushed past 1.0 so the
    // bloom pass finds it, which is what lets a blocked thread read as red from a distance
    // at which the character itself is a dozen pixels tall.
    const glowMat = new THREE.MeshBasicMaterial({ color: 0xffffff, toneMapped: true })
    parts.orb = this._mesh(new THREE.SphereGeometry(R * 0.17, 8, 6), glowMat, capacity, false)

    // The hammer, held in the right hand while a thread is running.
    parts.hammer = this._mesh(
      hammerGeometry(R),
      toon({ vertexColors: true }),
      capacity,
      true
    )
    this.variantOf.set('hammer', 'hammer')

    // Face: the features only, drawn straight onto the head beneath. Built as a sphere cap
    // a hair larger than the head, so it lies exactly on the curved surface instead of
    // clipping through it — a flat plane at this radius sinks inside the sphere and the
    // features disappear.
    const faceGeo = sphereCap(P.helmetR * 1.052, 1.9, 1.12, 18, 12)
    // The head is squashed into a skull, so the face has to be squashed with it or the
    // features sit off the surface at the cheeks.
    faceGeo.scale(0.97, 1.06, 0.94)
  ...
  setRig(rig) {
    ...
    // The body is cel-shaded like everything worn on it. Without this the hair would be
    // banded and the torso underneath smoothly lit, and the character would come apart into
    // two different drawings at the neck.
    const material = decorateSkinned(
      new THREE.MeshToonMaterial({ color: 0xffffff, gradientMap: this.gradient }),
      this.crewUniforms
    )
  ...
  /**
   * The face material, and the second half of what makes these read as anime faces.
   *
   * The atlas is a three-channel mask (see `faces.js`) and this shader composites it in a
   * fixed order onto whatever is behind: **ink** goes down as near-black line work, the
   * **iris** goes over it in the character's own eye colour pushed past 1.0, and the
   * **highlight** channel paints sclera and catchlights in white on top. Alpha is whichever
   * of the three is strongest at that texel, so the skin of the head shows through
   * everywhere the face is not drawn.
   *
   * That ordering is the whole trick. A single-channel mask can only tint one colour by
   * alpha, which is why the space version's faces were glowing dots on a dark screen: there
   * was no way to say "dark outline, bright iris, white catchlight" in one channel. Three
   * channels say it in one texture fetch.
   *
   * `depthWrite` is off because this is transparent: with it on, the cap would write depth
   * across its whole rectangle and punch a hole in anything drawn behind it later.
   */
  _faceMaterial() {
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      // The iris is lifted a little above its authored colour and no further. The space
      // version pushed this to 1.85 on top of colours already past 1.0, because a visor
      // *wanted* to glow; an eye that glows is an eye the bloom pass turns into a white
      // hole where the face used to be.
      shader.uniforms.uGlow = { value: 1.12 }
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace(
          ...
          `vec3 m = texture2D( map, vMapUv ).rgb;
           // The mask is drawn from paths, so its edges are already antialiased — taking
           // coverage straight from it is what gives the features soft edges against the
           // skin without a single extra sample.
           float ink = m.r;
           float iris = m.g;
           float lit = m.b;

           vec3 inkColor = vec3( 0.06, 0.05, 0.09 );
           vec3 irisColor = vColor.rgb * uGlow;

           // Painted in the order an eye is drawn: line work, then the coloured iris over
           // it, then the white of the sclera and the catchlights on top.
           vec3 col = inkColor;
           col = mix( col, irisColor, iris );
           // Not pure white: the sclera and the catchlights sit just under the bloom
           // threshold, so they read as bright without smearing across the whole face.
           col = mix( col, vec3( 0.94 ), lit );

           diffuseColor.rgb = col;
           diffuseColor.a = max( ink, max( iris, lit ) );`
        )
        // vColor is the iris source above, so the usual instance-colour multiply must go.
  ...
  _applyShadowFlags() {
    ...
    // The face is a transparent decal and the orb is unlit; neither has a silhouette worth
    // casting. Hair and props do — a spiky shadow is half of what sells the haircut.
    const noShadow = new Set(['face', 'orb', 'ahoge'])
    for (const [name, mesh] of Object.entries(this.parts)) {
      mesh.castShadow = on && !noShadow.has(name)
    }
  ...
  setRoster(entries, world) {
    ...
    for (const agent of this.agents) {
      // The player is not on the roster and never will be — it belongs to whoever is at the
      // keyboard, not to a thread — so a poll that does not mention it must not walk it back
      // into the ship.
   ...
   * You.
   *
   * The player is an ordinary member of the crew with one thing different: its velocity
   * comes from the keyboard instead of from the state machine. Everything else — the body,
   * the baked animation, hair, face, the ground tracking, the crowd separation, the
   * collision slide — is the same code every other character runs, which is why walking
   * around costs a hundred lines rather than a second character system.
   *
   * It is deliberately *not* given a spirit orb or a status badge: those mean "this thread
   * wants something", and the player is not a thread.
   */
  spawnPlayer(id = 'you') {
    const existing = this.byId.get(id)
    if (existing) return existing
    const agent = this._spawnAgent({ id, thread: null, status: 'idle', site: null, anchor: null })
    agent.player = true
    agent.state = 'player'
    agent.scale = 1
    agent.speed = PLAYER_WALK
    // Straight to a full-size character: the pop-out-of-the-ship ceremony is for arrivals,
    // and you did not arrive, you were always here.
  ...
  /** Where the player is, for the camera to pin itself to. */
  playerPosition() {
    return this.player && this.player.state !== 'gone' ? this.player.pos : null
  }

  /**
   * The nearest character the player could plausibly be talking to: close, and roughly in
   * front. The cone is generous on purpose — being made to line up precisely with something
   * that is itself walking about is the kind of precision no one enjoys.
   */
  nearestToPlayer(maxDist = TALK_RANGE) {
    const p = this.player
    if (!p) return null
    let best = null
    ...
    for (const agent of this.agents) {
      if (agent.player || agent.state === 'gone' || agent.scale < 0.5) continue
      const dx = agent.pos.x - p.pos.x
      const dz = agent.pos.z - p.pos.z
      const d = Math.hypot(dx, dz)
      if (d > maxDist) continue
      // Facing counts, but only as a tie-break: dead ahead is worth about a metre.
  ...
  /**
   * Drive the player from the input the page collected this frame.
   *
   * `input.x` and `input.z` are the intent in *camera* space, already rotated into world
   * space by the caller — the camera owns which way "forward" points, and having the
   * character work that out for itself would mean two places that both believe they know
   * where the camera is looking.
   */
  _playerMove(agent, dt) {
    const input = this.playerInput
    const mag = input ? Math.hypot(input.x, input.z) : 0

    if (mag > 0.01) {
      const want =
        (agent.riding ? PLAYER_RIDE : input.run ? PLAYER_RUN : PLAYER_WALK) * Math.min(1, mag)
      // Quicker to start than an agent, because a character you are steering that eases
      // into motion reads as lag rather than as weight.
      agent.vel.x = THREE.MathUtils.damp(agent.vel.x, (input.x / mag) * want, 14, dt)
      agent.vel.z = THREE.MathUtils.damp(agent.vel.z, (input.z / mag) * want, 14, dt)
      agent.targetYaw = Math.atan2(input.x, input.z)
    } else {
      // Stopping is quicker still. Anything slower and the character coasts past the thing
      // you stopped in order to talk to.
      agent.vel.x = THREE.MathUtils.damp(agent.vel.x, 0, 22, dt)
      agent.vel.z = THREE.MathUtils.damp(agent.vel.z, 0, 22, dt)
    }

    /**
     * The crowd still pushes the player, but only a quarter as hard as it pushes another
     * character.
     *
     * At full strength this is unplayable, and unplayable in a way that only appears with a
     * real thread list: you arrive next to the ship where seventy characters are milling
     * about, and seventy separation forces sum into a wall that holds you in place while
     * the walk animation plays. Measured, it was fourteen centimetres of travel in a second
     * and a half. Damping the player's *response* rather than its effect keeps the crowd
     * solid — they still shove each other, and you still shove them — while letting you
     * wade through a group instead of bouncing off it.
     */
    const push = this._separation(agent, this._sep).multiplyScalar(PLAYER_PUSHBACK)
    const dx = (agent.vel.x + push.x) * dt
    const dz = (agent.vel.z + push.z) * dt
    if (this.nav) {
      if (!this.nav.slide(agent.pos, dx, dz)) agent.vel.multiplyScalar(0.3)
    } else {
      agent.pos.x += dx
      agent.pos.z += dz
    }
  ...
  /**
   * Where a character first appears.
   *
   * Everyone used to walk out of the ship's door, which was the right flourish when the whole
   * colony was sixty units across and the longest walk home was twenty. At city scale the
   * gate is eighty to ninety units from most front doors — a minute and a half of walking —
   * and the walk state gives up long before that, so **every character in the colony timed
   * out in an empty field and adopted it as home**. The cities were built and nobody lived in
   * them.
   *
   * So a character now appears a short way down its own street and walks in. The arrival
   * still reads as an arrival, it just happens in the district the thread actually belongs
   * to. The ship keeps its meaning as the way *you* enter the world; it stops being the
   * turnstile every conversation in the colony has to queue at.
   */
  _spawnNear(site) {
    if (!site) return new THREE.Vector3()
  ...
  _spawnAgent(entry) {
    ...
    // The character. Everything about how this thread *looks* is derived from its id and
    // nothing else — the same chat is the same character across reloads, across polls and
    // across days, with nothing persisted anywhere to make that true.
    const theme = themeFor(entry.id)
    const h = hash(entry.id)

    const agent = {
      theme,
      /** The character's name, shown when you walk up to them. */
      charName: nameFor(entry.id, theme),
      hairMesh: 'hair_' + theme.hair,
      propMesh: 'prop_' + theme.prop,
      propSlot: propSpec(theme.prop)?.slot || 'head',
      propTint: propSpec(theme.prop)?.tint || 'trim',
      skin: SKIN_TONES[(h >>> 11) % SKIN_TONES.length],
      hairColor: new THREE.Color(theme.hairColor),
      outfit: new THREE.Color(theme.outfit),
      /** The ahoge's spring: an angle and its velocity, integrated in `_writeMatrices`. */
      ahoge: 0,
      ahogeVel: 0,
      lastYaw: 0,
      id: entry.id,
      ...
      workAt: 0,
      pos: entry.site
        ? this._spawnNear(entry.site)
        : (() => {
            const door = this.world?.shipDoor?.() || new THREE.Vector3(0, 0, 0)
            return new THREE.Vector3(door.x + jitter(), 0, door.z + jitter())
          })(),
      ...
      faceTimer: 0,
      faceIndex: 0,
      /** Iris colour: the theme's, not the status's. It says which chat, not what it is doing. */
      eye: new THREE.Color().setRGB(theme.eye[0], theme.eye[1], theme.eye[2]),
      /** Status colour, worn only by the spirit orb. */
      aura: new THREE.Color(0xffffff),
      hop: 0,
  ...
  _updateAgent(agent, entry) {
    ...
    // Somebody who has gone indoors keeps the site the colony gave them there. Without this
    // the next poll — fifteen seconds at most — hands back their outdoor site and walks them
    // straight back out of their own house while you are standing in it.
    if (agent.indoors) {
  ...
  /** Status change → new behaviour, new aura. The character itself never repaints. */
  _applyStatus(agent, status) {
    agent.aura.set(STATUS_AURA[status] || STATUS_AURA.idle)
  ...
  update(dt, elapsed) {
    ...
    // Before anybody moves: pairings decided here are read by `_step` on this same frame,
    // so a character never spends a frame walking somewhere its conversation has ended.
    if (!reduced) this.social.update(this.agents, dt, elapsed)
    ...
    this._writeMatrices(elapsed, anim, dt)
    return write
  }
  ...
  _step(agent, dt, elapsed, anim) {
    ...
    agent.blocked = false
    // Distance is always measured to the real goal; steering follows the route to it. The
    // player has no goal — it goes where it is steered — so it never asks the router for a
    // route, and never spends a slot of the per-frame path budget the crew is sharing.
    const steer = agent.player ? agent.pos : this._steerTarget(agent, this._wp)
    ...
    switch (agent.state) {
      ...
      case 'walking': {
        ...
        /**
         * How long this character is allowed to spend getting home.
         *
         * This used to be a flat forty-five seconds, which was generous when nothing in the
         * colony was more than twenty-five units from anything else and fatal the moment the
         * cities grew: a ninety-unit walk at two units a second takes longer than the timer,
         * so the timer fired first, every time, for every character — and the give-up branch
         * below *rewrites the character's home to wherever it happened to be standing*. One
         * constant left twenty-four cities empty and put seventy-three people in a field.
         *
         * The allowance is worked out once, when the walk starts, from the distance actually
         * being walked. Recomputing it from the *remaining* distance each frame reintroduces
         * the same bug wearing a different hat — the allowance shrinks as the character
         * approaches, so a long walk is condemned in its final few metres.
         */
        if (agent.walkAllowance == null) {
          agent.walkAllowance = 14 + (dist / Math.max(0.6, agent.speed)) * 2.4
        }
      ...
      case 'at-site': {
        const social = Social.intent(agent)
        if (social) {
          /**
           * Attending to somebody else takes precedence over pottering.
           *
           * A spectator still has to walk over, so it gets the ordinary walk; a character
           * already in conversation just stops and turns. Either way the drift is skipped
           * entirely rather than blended with — a character half-wandering and
           * half-listening reads as one that cannot decide, which is worse than either.
           */
          if (social.approach) {
            const to = this._v.set(social.look.x - agent.pos.x, 0, social.look.z - agent.pos.z)
            this._walk(agent, to, Math.max(0.6, to.length() - 1.6), dt, 0.85)
          } else {
            agent.vel.set(0, 0, 0)
            this._settle(agent, dt)
          }
          this._faceToward(agent, social.look, dt)
      ...
      // You. Velocity comes from the keyboard; everything below the switch — the ground,
      // the turn, the clip choice — is shared with the rest of the crew.
      case 'player': {
        this._playerMove(agent, dt)
        break
      }
    ...
    // A rider sits on top of what is carrying them. Added after the ground is resolved so
    // the mount still follows the terrain underneath rather than floating at a fixed height.
    agent.pos.y = (agent.groundY || 0) + agent.hop + (agent.riding ? RIDE_LIFT : 0)
  }
  ...
  _animate(agent, dt, anim) {
    ...
    if (agent.state === 'spawning') key = 'spawn'
      ...
      // Whatever the conversation is about. Reading it off the social state rather than the
      // status keeps the two apart: status says what the *thread* is doing, this says what
      // the character is doing about somebody else's.
      key = Social.intent(agent)?.clip || 'idle'
    } else {
      switch (agent.status) {
        case 'working':
          key = 'work'
  ...
  _writeMatrices(elapsed, anim, dt) {
    const parts = this.parts
    const { head, ahoge, orb, face, hammer } = parts
    const rig = this.rig
    ...
    // Per-frame instance counters for the meshes only some characters occupy. Reused
    // between frames rather than reallocated, because this runs every frame forever.
    const counts = (this._counts ||= new Map())
    for (const name of this.variantOf.keys()) counts.set(name, 0)

    /**
     * Only the crew near enough to be people.
     *
     * The crew is one instanced draw, which is what makes seventy-odd characters affordable
     * at all — and also what stops the GPU skipping the ones on the far side of the colony:
     * every instance pays its 5,950 triangles whether it is in front of you or four districts
     * away, once for the picture and again for the shadow map. Measured, that was 880k
     * triangles a frame, the largest single cost in the scene, most of it spent on people two
     * pixels tall.
     *
     * So the write loop skips them. The cutoff sits just past the lattice spacing, which puts
     * the boundary in the parkland between districts rather than inside one — nobody blinks
     * out of the street you are standing in, and by the time a district is far enough for its
     * people to go it has already dropped to a skyline (`FAR_DETAIL`), so there is nothing
     * left there to look empty.
     */
    const view = this.viewPoint
    for (const agent of this.agents) {
      if (agent.state === 'gone') continue
      if (view && !agent.player) {
        const dx = agent.pos.x - view.x
        const dz = agent.pos.z - view.z
        if (dx * dx + dz * dz > CREW_DRAW_RANGE * CREW_DRAW_RANGE) continue
      }
      // Hidden, not removed: the player in first person still walks, still pushes through
      // crowds and is still what the camera is pinned to — it simply is not drawn, because
      // what it would draw is the inside of your own head.
      ...
      // Everything worn hangs off a bone at the frame the body is actually on, so hair
      // cannot drift off a head that is looking down or lying on the ground.
      if (rig) {
        // Head, hair, face, ahoge. The anime geometry bakes its own height above the head
        // bone (see `HEAD_UP` in `anime-parts.js`), so these are placed at the bone itself
        // rather than offset again — offsetting twice puts the hair a head above the head.
        attachMatrixAt(rig, agent.frame, this.headSlot, bone)
        const headWorn = this._m5.multiplyMatrices(root, bone)
        setPart(child, headWorn, head, i, 0, 0, 0, 0, 0, 0)
        // A shade below the head's own centre. Anime faces sit low on the skull — the
        // forehead is the tall part — and a face centred on the sphere reads as a very
        // surprised egg.
        setPart(child, headWorn, face, i, 0, P.headUp - P.helmetR * 0.12, 0, 0, 0, 0)

        const hairMesh = parts[agent.hairMesh]
        if (hairMesh) {
          const slot = counts.get(agent.hairMesh)
          counts.set(agent.hairMesh, slot + 1)
          setPart(child, headWorn, hairMesh, slot, 0, 0, 0, 0, 0, 0)
          if (hairMesh.instanceColor) {
            hairMesh.setColorAt(slot, agent.hairColor)
            hairMesh.instanceColor.needsUpdate = true
          }
        }

        // The ahoge lags the turn and springs back — a damped spring driven by how fast the
        // character is actually rotating. It is one bone's worth of secondary motion and it
        // is the difference between a model standing there and a character standing there.
        let dYaw = agent.yaw - agent.lastYaw
        // Unwrap, or a turn across the ±π seam flings it round like a propeller.
        if (dYaw > Math.PI) dYaw -= Math.PI * 2
        else if (dYaw < -Math.PI) dYaw += Math.PI * 2
        agent.lastYaw = agent.yaw
        agent.ahogeVel += (-agent.ahoge * 42 - agent.ahogeVel * 7.5) * dt - dYaw * 2.4
        agent.ahoge = THREE.MathUtils.clamp(agent.ahoge + agent.ahogeVel * dt * anim, -0.9, 0.9)
        const sway = agent.ahoge + Math.sin(elapsed * 1.7 + agent.phase) * 0.06
        setPart(child, headWorn, ahoge, i, 0, P.headUp + P.helmetR * 0.92, -P.helmetR * 0.1, 0.3, 0, sway)

        // The orb rides at the shoulder, orbiting slowly so it reads as floating rather
        // than bolted on.
        attachMatrixAt(rig, agent.frame, this.chestSlot, bone)
        const chestWorn = this._m6.multiplyMatrices(root, bone)
        const orbit = elapsed * 1.15 + agent.phase
        setPart(
          child,
          chestWorn,
          orb,
          i,
          Math.cos(orbit) * 0.62,
          0.42 + Math.sin(orbit * 1.7) * 0.08,
          Math.sin(orbit) * 0.34 - 0.1,
          0,
          0,
          0
        )

        // The genre prop, on whichever bone its entry named.
        const propMesh = parts[agent.propMesh]
        if (propMesh) {
          const slot = counts.get(agent.propMesh)
          // A hand prop only exists while there is a hand pose to hold it — otherwise it
          // hangs in the air beside an idling character.
          const inHand = agent.propSlot === 'hand'
          if (!inHand || agent.clipKey !== 'work') {
            counts.set(agent.propMesh, slot + 1)
            let base = headWorn
            if (agent.propSlot === 'chest') base = chestWorn
            else if (inHand) {
              attachMatrixAt(rig, agent.frame, this.handSlot, bone)
              base = this._m7.multiplyMatrices(root, bone)
            }
            const rx = inHand ? P.gripRx : 0
            const rz = inHand ? P.gripRz : 0
            const x = inHand ? P.gripX : 0
            const y = inHand ? P.gripY : 0
            const z = inHand ? P.gripZ : 0
            setPart(child, base, propMesh, slot, x, y, z, rx, 0, rz)
            if (propMesh.instanceColor) {
              propMesh.setColorAt(slot, this._propColor(agent))
        ...
        if (agent.clipKey === 'work') {
          ...
          const slot = counts.get('hammer')
          counts.set('hammer', slot + 1)
          setPart(child, worn, hammer, slot, P.gripX, P.gripY, P.gripZ, P.gripRx, 0, P.gripRz)
        }
      }

      // Outfit, skin and iris never change once a character exists — they are the theme,
      // and the theme is a pure function of the thread id. So they are written only when an
      // agent leaving the roster shuffles everyone's slot along, not on every frame. (Hair
      // and props are the exception and *are* written per frame, because their own instance
      // slots are re-packed every frame as characters come and go.)
      const c = this._color
      if (agent.index !== i || agent.colorDirty) {
        agent.colorDirty = false
        crew?.setColorAt(i, agent.outfit)
        head.setColorAt(i, c.setHex(agent.skin))
        ahoge.setColorAt(i, agent.hairColor)
        face.setColorAt(i, agent.eye)
        staticDirty = true
      }

      // The orb pulses; a blocked agent's stutters like a fault light.
      const pulse =
        agent.status === 'blocked'
          ? (Math.sin(elapsed * 9) > 0.2 ? 1 : 0.05)
          : 0.55 + 0.45 * Math.sin(elapsed * 2.6 + agent.phase)
      orb.setColorAt(i, c.copy(agent.aura).multiplyScalar(0.7 + pulse * 1.7))
    ...
    const n = i
    // The orb pulses every frame; the rest only re-upload when something moved slot. Hair
    // and props flag themselves above, as they are written.
    for (const [name, mesh] of Object.entries(this.parts)) {
      // A variant mesh is only as long as the number of characters that actually wear it;
      // everything else covers the whole crew.
      mesh.count = this.variantOf.has(name) ? counts.get(name) || 0 : n
      mesh.instanceMatrix.needsUpdate = true
      if (mesh.instanceColor && (staticDirty || name === 'orb')) mesh.instanceColor.needsUpdate = true
    }
    if (crew) {
      crew.count = n
  ...
  /**
   * What colour a character's prop is painted. Each prop entry names the part of the
   * theme it borrows from rather than carrying a colour of its own, so a headband is always
   * the same red as the trim on the building its thread is putting up.
   */
  _propColor(agent) {
    const c = this._propC
    switch (agent.propTint) {
      case 'outfit':
        return c.copy(agent.outfit)
      case 'hair':
        return c.copy(agent.hairColor)
      case 'ink':
        return c.setHex(0x1a1a22)
      default:
        return c.setHex(agent.theme.trim)
    }
  }
  ...
  pick(camera, ndcX, ndcY, aspect, maxDist = 0.075) {
    ...
    for (const agent of this.agents) {
      // You are not clickable. Selecting yourself would open a thread card for a thread
      // that does not exist, and in walk mode you are under the cursor constantly.
      if (agent.player || agent.scale < 0.3 || agent.state === 'gone') continue
