...
export class Indicators {
  ...
  update(agents, elapsed, statusFor) {
    ...
    for (const agent of agents) {
      ...
      // The player never gets a badge: a badge means "this thread wants something from
      // you", and you are not a thread.
      if (agent.player || agent.scale < 0.4 || agent.state === 'gone') continue
