 ...
 */
/**
 * How long the adaptive scaler holds still after changing the render scale.
 *
 * Each change costs a rebuilt post chain and a redrawn frame, so they have to be rare. Ten
 * seconds is long enough that a run of slow frames produces one step rather than a
 * staircase, and short enough that a machine that genuinely cannot keep up still reaches a
 * comfortable scale within a minute.
 */
const SCALE_SETTLE_MS = 10000
...
export class Engine {
  constructor(settings) {
    ...
    this.camera = new THREE.PerspectiveCamera(settings.get('fov'), 1, 0.5, 3000)
    ...
    this.canvas.classList.add('anime-crossing-canvas')
  ...
  _governQuality() {
    ...
    // Still settling from the last change: the frames right after a resize are not
    // representative anyway, since one of them included the resize.
    if (now < (this._settleUntil || 0)) return
    ...
    /**
     * Drowning is not the same as struggling, and the two want different responses.
     *
     * The original rule waited three slow seconds and then took one small step down, with a
     * long settle after it. That is right for a machine hovering just under the target — it
     * finds the boundary without oscillating. It is badly wrong for a machine at twenty
     * frames a second: the same caution means a dozen steps and the better part of two
     * minutes of unusable framerate before the scale is anywhere near right, and by then
     * nobody is still watching.
     *
     * So a really bad reading skips the wait and takes a much larger step. The evidence is
     * already unambiguous — nothing about a second sample at 20fps is going to change the
     * decision, and every second spent confirming it is a second of the same bad frames.
     */
    const drowning = fps < 30

    let next = current
    if (drowning || this._slow >= 3) {
      next = Math.max(floor, current - (drowning ? 0.3 : 0.15) * dpr)
      this._slow = 0
    ...
    if (Math.abs(next - current) > 0.01) {
      ...
      /**
       * Fill the new targets before the browser gets to composite them.
       *
       * Resizing the post chain allocates fresh render targets, and a fresh target is
       * *empty*. The frame that has already been drawn this tick went to the old ones, so
       * without this the compositor is handed a blank buffer and the whole window flashes
       * black for exactly one frame. That flash is the one people notice, because it is the
       * only time the entire screen changes at once.
       *
       * Drawing one more frame immediately costs a few milliseconds on a machine that was
       * already struggling, and buys back the artefact that made the struggle visible.
       */
      this.renderFrame()

      /**
       * Hold still afterwards — but only briefly when things are genuinely bad.
       *
       * The long settle exists to stop a machine near the threshold flashing black every few
       * seconds as it hunts. A machine at twenty frames is not hunting, and making it wait
       * ten seconds between each step is the same mistake as making it wait for a second
       * sample: it turns a two-step fix into twenty seconds of bad frames.
       */
      this._settleUntil = now + (drowning ? SCALE_SETTLE_MS * 0.25 : SCALE_SETTLE_MS)
    }
  }
