...
export class Particles {
   ...
   * Weather.
   *
   * Every world names its own in its preset — petals over Hanami Hills, rain in
   * Neo-Akihabara, spirit lights rising through the teal dusk, embers off the festival
   * lanterns, seed fluff over the Skyward Isles, snow in winter. It is the cheapest
   * immersion in the whole project: a scene with nothing moving in the air reads as a
   * model of a place, and the same scene with something drifting through it reads as the
   * place itself, for a few hundred points and no draw calls of its own.
   *
   * Spawned in a ring around the *camera* rather than over the world, so the density you
   * see is the density near you at any zoom. Falling weather starts above and comes down;
   * rising weather starts at knee height and goes up, which is the whole difference between
   * snow and fireflies.
   */
  ambient(dt, camera, planet) {
    ...
    // Half rate on the lighter preset. Weather is the first thing that should thin out when
    // a machine is struggling, because it is the only thing on screen carrying no meaning.
    this._ambientTimer = w.rate * (this.settings.get('particles') === 'full' ? 1 : 2.4)

    const a = Math.random() * Math.PI * 2
    const r = 4 + Math.random() * 30
    const x = camera.position.x + Math.cos(a) * r
    const z = camera.position.z + Math.sin(a) * r
    const rising = w.kind === 'rise'
    const y = rising ? 0.3 + Math.random() * w.high : 5 + Math.random() * w.high

    const layer = w.glow ? this.glow : this.dust
    layer.spawn(
      x,
      y,
      z,
      (Math.random() - 0.5) * w.sway,
      // Downward for anything falling, upward for anything rising. The sign is the only
      // thing separating the two kinds; everything else is colour and rate.
      -w.fall * (0.75 + Math.random() * 0.5),
      (Math.random() - 0.5) * w.sway,
      w.color[0],
      w.color[1],
      w.color[2],
      w.size * (0.7 + Math.random() * 0.6),
      rising ? 2.5 + Math.random() * 2.5 : 5 + Math.random() * 4,
      // Almost no drag on rain so it stays in straight lines; plenty on anything that is
      // supposed to wander.
      w.fall > 3 ? 0.05 : 0.5,
      0,
      // The floor is only ever tested against a downward velocity, so rising weather is
      // given one far below the world purely so the number says what it means.
      rising ? -50 : 0.05
    )
  }
