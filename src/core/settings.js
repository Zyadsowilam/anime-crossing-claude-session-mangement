...
const STORE_KEY = 'animecrossing.settings.v1'
 ...
 *
 * "High" rather than "Ultra", and the difference between them is one number: Ultra renders at
 * 1.5x the display's own resolution and then scales it down. That is 2.25 times the pixels —
 * through bloom and depth of field, which are both full-screen passes — for an anti-aliasing
 * improvement, and it was a reasonable default when a zone was a platform with seven huts on
 * it. At city scale it is the single most expensive setting in the app and the first thing
 * anyone reporting a low frame rate is unknowingly paying for.
 *
 * Every other knob is identical between the two presets: same shadows, same textures, same
 * scatter, same effects. Ultra is still there for anyone with the headroom to want it.
 */
...
const DEFAULTS = {
  ...
  ...PRESETS[DEFAULT_PRESET].values,

  // World
  planet: 'sakura',
  /**
   * Sound. On, but the graph does not exist until the page is first touched — browsers
   * refuse to start an AudioContext before a gesture, which conveniently means nothing can
   * make a noise at somebody who opened the page and walked away.
   */
  sound: true,
  ...
  exposure: 1.0,
  bloomStrength: 0.25,
  tiltShiftStrength: 0.15, // 0..1 — share of the effect's full blur radius (2% of frame height)
