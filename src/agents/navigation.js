 ...
 *
 * Coarsened along with the world. At half a unit, a grid covering a colony this size is two
 * and a half million cells and forty megabytes of typed arrays, rebuilt every time the roster
 * changes. Two thirds of a unit is a quarter of that and still resolves the streets, which are
 * four units of clear paving between frontages — the narrowest gap anyone has to walk down.
 */
const CELL = 0.66
/** Half-width of the navigable square. Comfortably contains the colony and the landing pad. */
/**
 * Half-extent of the walkable grid, in world units.
 *
 * It has to cover every deck plus the ground between them, and the lattice opened out to
 * eleven units a cell — a colony nine cells across now reaches past seventy. Sized short,
 * the outermost zones sit off the edge of the grid, every cell out there reads as blocked,
 * and their characters simply refuse to walk.
 */
/**
 * Half-extent of the walkable grid, in world units.
 *
 * Sized to the whole colony plus the country between the cities: at seventeen units a
 * lattice cell, a nine-cell map reaches past a hundred and twenty. Undersized, the outermost
 * cities fall off the edge of the grid, every cell out there reads as blocked, and nobody
 * who lives in them can move at all.
 */
const HALF = 290
/** Give up rather than stall the frame if a search goes pathological. */
// A path across a city is a longer search than a path across a village.
