...
/**
 * A scheduled task starts a *fresh* thread every time it fires, so a routine that has run
 * fifteen times arrives as fifteen threads — fifteen astronauts on one plot, all of them the
 * same agent doing the same job on a timer. That buries the threads you actually talk to.
 *
 * So a routine folds down to the one thread that stands for it. The newest run carries the
 * identity, because that is the run you want when you click it; everything that describes
 * *the schedule* rather than one firing of it is aggregated — the transcript sizes sum, so
 * the building measures the whole job, and any run that is running, unread or errored makes
 * the agent so. Archiving reaches every run behind it, and a routine is only archived once
 * all of its runs are.
 */
function collapseRoutines(threads) {
  ...
  for (const thread of threads) {
    ...
    const runs = groups.get(thread.routine)
    if (runs) {
      runs.push(thread)
    } else {
      groups.set(thread.routine, [thread])
      out.push(thread) // hold the routine's place in the order; rewritten below
    }
  }

  const folded = new Map()
  for (const [routine, runs] of groups) {
    if (runs.length === 1) continue
    // Newest first: the run whose id, title and transcript the agent takes on.
    runs.sort((a, b) => (b.lastActivityAt || 0) - (a.lastActivityAt || 0))
    const any = (key) => runs.some((r) => r[key])
    folded.set(routine, {
      ...runs[0],
      runCount: runs.length,
      sizeBytes: runs.reduce((sum, r) => sum + (r.sizeBytes || 0), 0),
      createdAt: Math.min(...runs.map((r) => r.createdAt || Infinity)) || 0,
      lastActivityAt: Math.max(...runs.map((r) => r.lastActivityAt || 0)),
      running: any('running'),
      unread: any('unread'),
      hasError: any('hasError'),
      starred: any('starred'),
      archived: runs.every((r) => r.archived),
      desktopSessionIds: [...new Set(runs.flatMap((r) => r.desktopSessionIds))],
    })
  }

  return out.map((thread) => (thread.routine && folded.get(thread.routine)) || thread)
}
...
async function scanThreads() {
  ...
  // After the per-thread state above, so a routine aggregates settled answers rather than
  // recomputing `unread` against a run whose focus history is not the schedule's.
  return collapseRoutines(threads).map(toThread)
}
